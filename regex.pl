...
# Matching a string pattern is done by the m// operator and the =~ binding operator. 
# The expression $string =~ m/$regexp/ returns true if the scalar $string matches the 
# pattern defined by the value of the scalar $regexp. The match operator supports its 
# own set of optional modifiers, written after the m// operator. The modifiers are 
# letters which indicate variations on the regexp processing. For example:

# $string =~ m/$regexp/i

# will make the match case insensitive.
my $name = "Josef Stalin";
if ($name =~ m/jo/) {
  print "Match found\n"
} else {
...
# m/bob[b-e]/ matches any string containg bobb or bobc or bobd or bobe

# Replacing a matched string
# Replacing a matched string with some other string is done by the substitute 
# operator s///. The basic form of the operator is s/REGEXP/REPLACEMENT/MODIFIER;. 
# The REGEXP is the regular expression for the string that we are looking for. 
# The REPLACEMENT is a specification for the text or regular expression that we 
# want to use to replace the found text with. The MODIFIER is the optional substitute operator modifier letter.
my $x = "My favorite pet is a cat. I must duplicate it.";
my $y = $x;
$y =~ s/cat/dog/;
print "$y\n";
$y = $x;
...
## Backtracking
# Parenthesised patterns have a useful property. When pattern matching is successful, the matching substrings 
# corresponding to the parenthesised parts are saved, which allow you to use them in further operations. 
# The matched value of the first parenthesised pattern is refered to as $1, the second as $2, and so on. For example:
my $number = "Telephone:   1234-5678";
if ($number =~ m/^Telephone:\s*(\d{4}-\d{4})$/) {
  print "The telephone number extracted is $1\n";
}
my $date = "Date:    2014-jun-01";
...
my $my_text = "You should try to use regular expressions while drinking an Espresso";
# Matching text
my $match_my_text = "express";
